...
| `interop/attenu-guard-0.15.0-envelopes` | observer-envelope verification by the author's Python runner over the 19 envelope vectors at revision envelope_vectors_v1.2 | the 19 accept and reject verdicts, their failure sets and every entry's witness-signed or process-asserted state under attenu-guard 0.15.0's own `verify_bundle` | `pkg/bin/python` with attenu-guard==0.15.0 installed, per SOURCE.md direction 1 |
| `interop/attenu-guard-0.15.0-envelopes` | observer-envelope verification by the author's TypeScript runner over the same 19 vectors | the same 19 verdicts, failure sets and states under the npm package's `verifyBundle` | `node run.js <envelope_vectors_v1.json>` with attenu-guard@0.9.0 installed, per SOURCE.md direction 1 |
