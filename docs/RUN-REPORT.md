...
3. `npm test`. After dependencies are installed the run makes no network calls. Exit 0
   means every APS-native vector passed. Families under `fixtures/cross-stack/` are not
   executed by `npm test`; the `cross-stack` CI job runs every verifier declared
   executable in `fixtures/cross-stack/index.json`, and `npm run verify:cross-stack`
   runs the same set locally. `npm run verify` is the generic APS corpus runner alone.
   To report on one executable external-system family, run the script its registry
   entry declares and paste that output instead.
