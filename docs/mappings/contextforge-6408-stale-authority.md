...
| code read | `IBM/mcp-context-forge` `main` at `380469fc2d50df599124b58c9ddea8ab2d739207` |
...
- `DecisionCache.invalidate` (`cache.py:261`) has no caller outside `cache.py`. The native engine loads its rules once at startup (`plugins/unified_pdp/engines/native_engine.py:86`). The OPA engine asks an external OPA server on each uncached request (`plugins/unified_pdp/engines/opa_engine.py:211`), so a policy updated in OPA takes effect there without a gateway restart, and a decision ContextForge already cached for that request is not evicted by the update. For that path the time for a policy change to reach a cached request is bounded by cache lifetime rather than by an invalidation event. The cache key includes the full subject, roles included (`cache.py:38` to `70`), so a change to the subject's roles produces a new key rather than a stale hit.
