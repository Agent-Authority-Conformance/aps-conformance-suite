...
//
// TypeScript runner for the lifecycle-principal-events family.
//
//   npx tsx fixtures/lifecycle-principal-events/verify.ts
...
const FAMILY = 'lifecycle-principal-events'
...
const UNSUPPORTED_CONCEPTS = [
  'grant terms outside the seven closed AuthorityVectorV1 facets (durability, relationship binding, coupled-interest claim, successor order)',
  'principal-event records (death, incapacity, dissolution filing or decree, forfeiture finding, guardian appointment, court order, agent exit)',
...
for (const vector of vectors.cases) {
  ...
  if (vector.prior_receipt !== undefined) {
    const prior = gate(fixture, {
      ...
      actor: vector.prior_receipt.actor,
