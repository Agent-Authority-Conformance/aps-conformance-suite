...
Each concern subdirectory ships a `run.mjs` transplanted from mcp-audit-gateway's own verifier at v0.6.0: `canonicalization/run.mjs` from `test/vectors/verify.mjs`, `checkpoint/run.mjs` from `test/vectors/verify-checkpoint.mjs`. The adapter inlines the canonicalization logic and uses only Node's stdlib (`node:crypto`, `node:fs`), so no `npm install` is needed to reproduce and the runner has no runtime dependency on the source package. Every recompute here is Mode A per CONTRIBUTING.md: reproduction of the documented verifier, transplanted, not an alternate implementation.
...
- canonicalization/ tuple-array canonical form for signed audit records, 8 record-level vectors (`genesis_all_fields`, `genesis_null_optionals`, `error_with_code`, `zero_duration`, `invalid_params_error`, `unicode_in_fields`, `max_safe_integer_duration`, `empty_string_tool_name`) exercising field order, null rule, error paths, unicode handling, boundary durations, and the empty-tool-name case (`canonicalization` block); elang2; Mode A; author-produced (author of the vectors and of `mcp-audit-gateway`); `canonicalization/run.mjs`, a transplant of upstream `test/vectors/verify.mjs` at commit a0f14a0…, 8/8
...
Recomputation agreement only. Passing every vector in this family establishes that a verifier agrees with mcp-audit-gateway v0.6.0 on the byte-level canonical form, the hash-chain relations, and the two completeness outcomes the vectors exercise, `head_missing` and `sequence_regression`. `count_mismatch` is documented here and not exercised. It does not establish that any policy was applied, that any external checkpoint was actually externalized to a consumer, or that any operator ever anchored a chain head to an external transparency log.
...
Each `results.json` is the recorded run and is never rewritten by the runner. An ordinary `node run.mjs` recomputes everything and compares it against that file, excluding the `ran_at`, `node_version` and `platform` run-instance stamps, which change on every run and on every machine; it exits non-zero if any other field differs. A reviewer wanting a fresh observation writes it elsewhere with `--output <path>`. The two committed files are one run each, both from 2026-09-02T04:43Z on Node 22.20.0 darwin; the verbatim block above is the earlier node:20-alpine run of 2026-08-31, and the two agree on every per-check outcome.
